...
(:
 ...
 :)
 (: change the page id system FG) :)
 
(:~
 : Enveloppe autour de nav:get-next / nav:get-previous : si le résultat
 : "naturel" atterrit sur un div SANS xml:id propre (peu importe son
 : @type, et peu importe s'il s'agit d'un conteneur wrapper ou d'un
 : simple bloc interne comme dans book-introduction), on continue à
 : avancer/reculer jusqu'à atterrir sur quelque chose d'adressable.
 : Récursif, gère n'importe quel nombre de blocs intermédiaires.
 :)
declare function config:custom-get-next($config as map(*), $div as element(), $view as xs:string) {
    let $next := nav:get-next($config, $div, $view)
    return
        if (exists($next) and empty($next/@xml:id)) then
            config:custom-get-next($config, $next, $view)
        else
            $next
};

declare function config:custom-get-previous($config as map(*), $div as element(), $view as xs:string) {
    let $prev := nav:get-previous($config, $div, $view)
    return
        if (exists($prev) and empty($prev/@xml:id)) then
            config:custom-get-previous($config, $prev, $view)
        else
            $prev
};
...
declare variable $config:next-page := config:custom-get-next#3;
(:
...
declare variable $config:previous-page := config:custom-get-previous#3;
...
(:insersion des variables pour les facettes le 13.07.2026 FG :)
declare variable $config:default-fields := ();

(:
 : Display configuration for facets to be shown in the sidebar. The facets
 : themselves are configured in the index configuration, collection.xconf,
 : and extracted from teiHeader via index.xql (idx:get-metadata).
 :)
declare variable $config:facets := [
    ...
    map {
        "dimension": "printer",
        "heading": "facets.printer",
        "max": (),
        "hierarchical": false()
    },
    map {
        "dimension": "place",
        "heading": "facets.places",
        "max": (),
        "hierarchical": false()
    },
    map {
    ...
    map {
        ...
        "heading": "facets.transcription-quality",
        "max": (),
        "hierarchical": false()
    },
    map {
        "dimension": "epistle",
        "heading": "facets.epistle",
        "max": (),
        "hierarchical": false(),
