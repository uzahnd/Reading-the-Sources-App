...
(:~
 : Trie les valeurs d'une facette. Si le config de la facette fournit une
 : propriété "order" (séquence de valeurs dans l'ordre voulu, ex. l'ordre
 : canonique de la Vulgate pour les épîtres), on trie selon cet ordre —
 : les valeurs absentes de la liste sont reléguées à la fin, par ordre
 : alphabétique entre elles. Sinon, comportement inchangé : tri
    ...
    array {
        if (exists($facets)) then
            if (map:contains($config, "order")) then
                ...
                let $pos := index-of($order, $key)
                let $sortKey := if (exists($pos)) then $pos[1] else count($order) + 1
                order by $sortKey ascending, $key ascending
                return
                    map { $key: $value }
            else
                for $key in map:keys($facets)
...
declare function facets:print-table($config as map(*), $nodes as element()+, $values as xs:string*, $params as xs:string*) {
    ...
    (: Si la facette fournit un ordre canonique (ex: épîtres pauliniennes),
       on complète avec les valeurs absentes des résultats actuels, à
       fréquence 0 — pour toujours afficher toutes les possibilités,
       et laisser l'utilisateur réduire lui-même le champ en cochant. :)
    let $facets :=
        if (map:contains($config, "order")) then
            map:merge((
                $facets,
                for $key in $config?order?*[not(. = map:keys($facets))]
                return map:entry($key, 0)
            ))
        else
            $facets
    return
...
declare function facets:display($config as map(*), $nodes as element()+) {
    ...
    return (
        <details class="facet-dimension" data-dimension="facet-{$config?dimension}">
            {
                if ($config?max != 0) then
                    <summary><pb-i18n key="{$config?heading}">{$config?heading}</pb-i18n>
                    {
                        if ($fcount > $max) then
                            <label>
